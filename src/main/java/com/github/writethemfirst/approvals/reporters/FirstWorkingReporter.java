...
public class FirstWorkingReporter implements Reporter {
     ...
     *
     * Since the {@link FirstWorkingReporter} is usualy used as a fallback reporter, in case no first working reporter
    ...
    public void mismatch(final Path approved, final Path received) {
        if (firstWorking().isPresent()) {
            firstWorking().get().mismatch(approved, received);
        } else {
            System.err.println(format("No working reporter for reporting mismatch between %s and %s", approved, received));
        }
    }
